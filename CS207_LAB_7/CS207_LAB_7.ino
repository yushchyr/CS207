...
int pattern2[] = {LOW, LOW, HIGH, HIGH, LOW, LOW, HIGH, HIGH};
int pattern3[] = {HIGH, HIGH, LOW, LOW, HIGH, HIGH, LOW, LOW};
int pattern4[] = {HIGH, HIGH, HIGH, LOW, HIGH, HIGH, HIGH, LOW};
...
void loop()                     // run over and over again
{
  ...
  //in again
  // larsonScanner();
  makePattern(ledPins, pattern1, 8, 200); // Function call
  makePattern(ledPins, pattern2, 8, 300);// Function call
  makePattern(ledPins, pattern3, 8, 400);// Function call
  makePattern(ledPins, pattern41, 8, 500);// Function call
}

void makePattern(int leds[], int pattern[], int num, int delayT)
{
  int delayTime = 200;
  for (int i = 0; i < num; i++)
  {
...
void larsonScanner() {
  ...
  for (led_Pin = 0;  led_Pin <= 7; led_Pin ++) {
    ...
    if (led_Pin == 0) {
      ...
      digitalWrite(ledPins[led_Pin - 1], LOW);
    delay(delayTime);
  }

  for (led_Pin = 6;  led_Pin >= 1; led_Pin --) {
    digitalWrite(ledPins[led_Pin], HIGH);
    digitalWrite(ledPins[led_Pin + 1], LOW);
    delay(delayTime);
  }
